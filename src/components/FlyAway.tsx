...
function ParallaxText({ children, baseVelocity = 100 }: ParallaxProps) {
  ...
  return (
    <div className="parallax">
      <motion.div className="scroller text-primary" style={{ x }}>
        <span>{children} </span>
        <span>{children} </span>
        <span>{children} </span>
        <span>{children} </span>
...
export default function FlyAway() {
  return (
    <section className="relative py-8 space-y-8">
        <ParallaxText baseVelocity={-3}>Gameverse</ParallaxText>
        <ParallaxText baseVelocity={3}>Cheap Great Unbeatable</ParallaxText>
    </section>
  )
}